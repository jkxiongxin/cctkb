...
public class Md5PwdEncoder implements PwdEncoder {
	...
	public String encodePassword(String rawPass, String salt) {
		...
		MessageDigest messageDigest = getMessageDigest();//得到MD5加密的对象
		byte[] digest;
		try {
			digest = messageDigest.digest(saltedPass.getBytes("UTF-8"));//加入混淆码后的密码进行MD5加密
		} catch (UnsupportedEncodingException e) {
			throw new IllegalStateException("UTF-8 not supported!");
		}
		return new String(Hex.encodeHex(digest));//进行数组水平转换
	}
